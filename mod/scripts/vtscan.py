...
def release_files(version):
    return [
        ("FlightFreedom-%s-DMM.zip" % version, "the DMM archive"),
        ("FlightFreedom-%s.zip" % version, "the full archive"),
        ("FlightFreedom.asi", "the plugin"),
    ]
...
def version_from_header():
    ...
    m = re.search(r'#define\s+FP_VERSION\s+"([^"]+)"', text)
    if not m:
        raise SystemExit("no FP_VERSION in %s" % h)
    return m.group(1)
...
def main():
    ...
    plugin = next((r for r in rows if r[0] == "FlightFreedom.asi" and r[3]), None)
