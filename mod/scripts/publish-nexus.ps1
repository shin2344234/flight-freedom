<#
.SYNOPSIS
    Publish the current release to the Flight Freedom mod page.
...
.DESCRIPTION
    ...
    does not depend on remembering an id.
        ...
        Mod id  38521561681312   the v3 id, not the 3488 in the page URL
        File id 7961412          the active "FlightFreedom ... DMM" entry

    Both were read back from the API on 2026-09-14 and are stable for the life
...
if ([string]::IsNullOrWhiteSpace($Version)) {
    ...
    $match  = Select-String -LiteralPath $header -Pattern '#define\s+FP_VERSION\s+"([^"]+)"'
    if (-not $match) { throw "No FP_VERSION in $header" }
    $Version = $match.Matches[0].Groups[1].Value
}

$archive   = Join-Path $mod  ("dist\FlightFreedom-{0}-DMM.zip" -f $Version)
...
$ids = @{
    FileId = '7961412'          # the active "FlightFreedom ... DMM" entry
    ModId  = '38521561681312'   # the v3 mod id, not the 3488 in the page URL
}
...
$args = @{
    ...
    ModId                     = $ids.ModId
    Version                   = $Version
    DisplayName               = ("FlightFreedom {0} DMM" -f $Version)
