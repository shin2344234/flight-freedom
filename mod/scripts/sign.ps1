<#
.SYNOPSIS
    Sign FlightFreedom.asi with Azure Trusted Signing, then verify it.
...
.EXAMPLE
    .\sign.ps1                       signs mod\dist\FlightFreedom.asi
...
if (-not $Path) { $Path = Join-Path $repo 'mod\dist\FlightFreedom.asi' }
