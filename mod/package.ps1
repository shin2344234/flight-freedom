...
#
# FlightFreedom-<version>.zip      manual install: plugin, ini, readme, licences
# FlightFreedom-<version>-DMM.zip  Definitive Mod Manager: the plugin alone,
...
$version = (Select-String -Path (Join-Path $here 'src\version.h') -Pattern '#define FP_VERSION\s+"([^"]+)"').Matches[0].Groups[1].Value
if (-not $version) { throw 'no FP_VERSION in src\version.h' }

$asi = Join-Path $dist 'FlightFreedom.asi'
$ini = Join-Path $dist 'FlightFreedom.ini'
...
$staging = Join-Path $env:TEMP "ff-package-$version"
...
$plain = Join-Path $dist "FlightFreedom-$version.zip"
$dmm = Join-Path $dist "FlightFreedom-$version-DMM.zip"
...
Write-Host "`nFlight Freedom $version"
