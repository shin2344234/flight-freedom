"""Check prose for AI hallmarks before it goes out as Seth's words.
    ...
    py -3 scripts/prose_check.py private/nexus/nexus-post-1.1.0.bbcode
...
def main():
    ...
    if "--all" in sys.argv or not args:
        ...
        args = []
        for sub in ("private/nexus", "private/github"):
            d = os.path.join(root, sub)
            if not os.path.isdir(d):
                continue
            args += [os.path.join(d, f) for f in sorted(os.listdir(d))
                     if f.endswith((".txt", ".bbcode", ".md")) and "template" not in f]
        args += [os.path.join(root, "README.md"), os.path.join(root, "mod", "README.md")]
